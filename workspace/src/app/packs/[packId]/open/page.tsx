...
export default function PackOpeningPage() {
  ...
  const pullCardsForOnePack = useCallback((): PokemonCard[] => {
    ...
    // Helper to pull random cards from a pool without replacement
    const pullRandomFrom = (pool: PokemonCard[], count: number): PokemonCard[] => {
      ...
      for (let i = 0; i < count; i++) {
        ...
        pulled.push(...mutablePool.splice(randomIndex, 1));
      }
      return pulled;
    };
    ...
    if (packData.id === 'destined-rivals-booster-001') {
      ...
      if (packData.rarityDistribution.rareSlot > 0) {
        ...
        } else if (rareSlotRoll < 0.35) { // 25% chance for Holo Rare
            rareSlotCard = pullRandomFrom(holoRarePool, 1)[0];
        } 
        
        // If the chosen rarity pool was empty or it was a normal rare roll
        if (!rareSlotCard) {
            rareSlotCard = pullRandomFrom(rarePool, 1)[0];
        }
  
        // Fallback chain if a pool was empty
        if (!rareSlotCard) {
    ...
    // Fill pack if still not full (due to exhausted pools)
    let fillAttempts = 0;
    const remainingCommons = availableCardsInSeries.filter(c => c.rarity === 'Common' && !packCards.some(pc => pc.id === c.id));
    while (packCards.length < packData.cardsPerPack && fillAttempts < 10) {
      const fillCard = pullRandomFrom(remainingCommons, 1)[0];
      if (fillCard) {
